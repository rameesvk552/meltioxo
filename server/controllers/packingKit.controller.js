...
const validatePayload = async (tenantId, body, transaction, currentId = null) => {
  const name = String(body.name || '').trim();
  const code = String(body.code || '').trim();
  const minimum = Number(body.minimum_fill_ml);
  const maximum = Number(body.maximum_fill_ml);
  const items = Array.isArray(body.items) ? body.items : [];
  if (!name) throw new AppError('Packing kit name is required', 400);
  if (!code) throw new AppError('Packing kit code is required', 400);
  if (!Number.isFinite(minimum) || minimum <= 0 || !Number.isFinite(maximum) || maximum < minimum) {
    throw new AppError('Enter a valid minimum and maximum fill capacity', 400);
  }
  ...
  return { name, code, minimum, maximum, items };
};
...
exports.getAll = async (req, res, next) => {
  try {
    ...
    res.json(await db.packingKit.findAll({ where, include, order: [['priority', 'DESC'], ['is_default', 'DESC'], ['name', 'ASC']] }));
...
exports.create = async (req, res, next) => {
  ...
  try {
    ...
    res.status(201).json(await db.packingKit.findByPk(kit.id, { include }));
...
exports.update = async (req, res, next) => {
  ...
  try {
    ...
    const values = await validatePayload(req.tenantId, req.body, transaction, kit.id);
    ...
    await db.packingKitItem.bulkCreate(values.items.map(row => ({ packing_kit_id: kit.id, packaging_material_id: row.packaging_material_id, quantity: Number(row.quantity) })), { transaction });
    await transaction.commit();
    res.json(await db.packingKit.findByPk(kit.id, { include }));
  } catch (error) { if (!transaction.finished) await transaction.rollback(); next(error); }
};
