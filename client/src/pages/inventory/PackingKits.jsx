...
export default function PackingKits() {
  ...
  const openKit = kit => {
    ...
    form.setFieldsValue(kit ? {
      name: kit.name, fill_quantity_ml: Number(kit.fill_quantity_ml ?? kit.minimum_fill_ml ?? kit.maximum_fill_ml),
    ...
    } : { fill_quantity_ml: 1, priority: 0, is_default: false, is_active: true, items: [{ quantity: 1 }] });
    setOpen(true);
  };
  ...
  const save = async values => {
    setSaving(true);
    try {
      const payload = { ...values, minimum_fill_ml: values.fill_quantity_ml, maximum_fill_ml: values.fill_quantity_ml, items: values.items || [] };
  ...
  const columns = [
    { title: 'Kit', render: (_, kit) => <Text strong>{kit.name}</Text> },
    { title: 'Fill quantity', render: (_, kit) => `${Number(kit.fill_quantity_ml ?? kit.minimum_fill_ml ?? kit.maximum_fill_ml)} ml` },
  ...
  return <div style={{ padding: 24 }}>
    <div style={{ display: 'flex', justifyContent: 'space-between', gap: 16, alignItems: 'center', marginBottom: 20 }}>
      <div><Title level={2} style={{ margin: 0 }}>Packing Kits</Title><Text type="secondary">Group bottles, caps, sprays, labels and boxes by fill quantity.</Text></div>
    ...
    <Modal title={editing ? 'Edit Packing Kit' : 'New Packing Kit'} open={open} onCancel={() => setOpen(false)} footer={null} width={720} destroyOnHidden>
      <Form form={form} layout="vertical" onFinish={save}>
        <Form.Item name="name" label="Kit name" rules={[{ required: true }]}><Input placeholder="30 ml Standard Kit" /></Form.Item>
        <Row gutter={12}><Col span={16}><Form.Item name="fill_quantity_ml" label="Fill quantity (ml)" rules={[{ required: true }]}><InputNumber min={0.0001} style={{ width: '100%' }} /></Form.Item></Col><Col span={8}><Form.Item name="priority" label="Priority"><InputNumber precision={0} style={{ width: '100%' }} /></Form.Item></Col></Row>
        ...
        <Row gutter={12}><Col span={8}><Form.Item name="is_default" label="Default" valuePropName="checked"><Switch /></Form.Item></Col><Col span={8}><Form.Item name="is_active" label="Available in POS" valuePropName="checked"><Switch /></Form.Item></Col></Row>
