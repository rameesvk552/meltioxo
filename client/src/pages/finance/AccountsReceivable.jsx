...
const AccountsReceivable = () => {
  const { data: report, loading } = useApiData('/reports/aging-receivables', { initialData: {} });
  ...
  return (
    <div style={{ padding: 24 }}>
      <Title level={2} style={{ fontFamily: "'Playfair Display', serif", color: 'var(--color-gold)', marginBottom: 24 }}>Accounts Receivable</Title>
      
      <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
        <Col xs={24} sm={12} lg={4}>
          <Card style={{ height: '100%' }}>
            <Statistic title={<span style={{ color: 'var(--color-text-secondary)' }}>Total Receivable</span>} value={agingData.reduce((sum, item) => sum + item.amount, 0)} prefix={<DollarOutlined />} styles={{ content: { color: '#52c41a' } }} />
          </Card>
        </Col>
