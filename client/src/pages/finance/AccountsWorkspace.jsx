...
const MAIN_TABS = ['Today', 'Money', 'Invoices', 'Reports', 'Reconcile', 'Ledger'];
...
function buildTree(accounts, search) {
  ...
  return ['asset', 'liability', 'equity', 'revenue', 'expense'].map(type => {
    const children = accounts.filter(account => account.type === type && !account.parent_id).map(node).filter(Boolean);
    return children.length ? { key: `type-${type}`, title: <strong>{typeLabel(type)}</strong>, children, selectable: false } : null;
  }).filter(Boolean);
}
...
export default function AccountsWorkspace() {
  ...
  const groups = accounts.filter(account => account.is_group);
  ...
  const receivable = Number(byCode('1100')?.balance || 0);
  const payable = Number(byCode('2000')?.balance || 0);
  ...
  const saveLedger = async values => {
    setSaving(true);
    try { await client.post('/accounts', values); message.success('Ledger created'); setLedgerModal(false); ledgerForm.resetFields(); await accountsQuery.reload(); }
  ...
  const renderContent = () => {
    ...
    if (mainTab === 'Reports') return <Row gutter={[16, 16]}>{[['Profit & loss', '/app/reports/profit-loss'], ['Balance sheet', '/app/reports/balance-sheet'], ['Trial balance', '/app/reports/trial-balance'], ['Stock valuation', '/app/reports/stock']].map(([title, route]) => <Col xs={24} sm={12} key={route}><Card hoverable onClick={() => navigate(route)}><FileTextOutlined /> <strong style={{ marginLeft: 8 }}>{title}</strong></Card></Col>)}</Row>;
  ...
  return <main className="accounts-page"><div className="accounts-heading"><div><span className="eyebrow">ACCOUNTING</span><h1>Accounts</h1></div><div className="quick-actions"><Button className="receipt-button" onClick={() => navigate('/app/payments/new')}>↙ Receipt</Button><Button className="payment-button" onClick={() => navigate('/app/payments/new')}>↗ Payment</Button></div></div><nav className="accounts-nav">{MAIN_TABS.map(tab => <button key={tab} className={mainTab === tab ? 'active' : ''} onClick={() => setMainTab(tab)}>{tab}</button>)}</nav>{errors.length > 0 && <Alert type="error" showIcon message="Some accounting data could not be loaded" description={errors[0]?.response?.data?.message || errors[0]?.message} style={{ marginBottom: 16 }} />}{renderContent()}
    <Modal title="New ledger" open={ledgerModal} onCancel={() => setLedgerModal(false)} onOk={() => ledgerForm.submit()} confirmLoading={saving} destroyOnHidden><Form form={ledgerForm} layout="vertical" onFinish={saveLedger} initialValues={{ type: 'asset', is_group: false }}><Row gutter={12}><Col span={8}><Form.Item name="code" label="Code" rules={[{ required: true }]}><Input /></Form.Item></Col><Col span={16}><Form.Item name="name" label="Ledger name" rules={[{ required: true }]}><Input /></Form.Item></Col></Row><Form.Item name="type" label="Account type" rules={[{ required: true }]}><Select options={['asset', 'liability', 'equity', 'revenue', 'expense'].map(value => ({ value, label: typeLabel(value) }))} /></Form.Item><Form.Item name="parent_id" label="Parent group"><Select allowClear showSearch optionFilterProp="label" options={groups.map(group => ({ value: group.id, label: `${group.code} — ${group.name}` }))} /></Form.Item><Form.Item name="is_group" label="This is a group" valuePropName="checked"><Switch /></Form.Item></Form></Modal>
